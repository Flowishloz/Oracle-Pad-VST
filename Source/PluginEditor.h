...
// ============================================================================
// LissajousOscilloscope — 60 Hz stipple-dot vectorscope (Osc1 L→X, R→Y).
// Reads from the processor's lock-free stereo FIFO; safe on the message thread.
// ============================================================================
class LissajousOscilloscope : public juce::Component, public juce::Timer
{
public:
    explicit LissajousOscilloscope (OraclePadAudioProcessor& p) : processor (p)
    {
        startTimerHz (60);
    }
    ~LissajousOscilloscope() override { stopTimer(); }
    ...
    void paint (juce::Graphics& g) override
    {
        ...
        const float hx = b.getWidth()  * 0.44f;
        const float hy = b.getHeight() * 0.44f;

        g.fillAll (juce::Colour (0xFF080A0F));

        // Drain FIFO completely each frame
        auto& fifo = processor.scopeFifo;
        auto* buf  = processor.scopeBuffer;
        int s1, n1, s2, n2;
        fifo.prepareToRead (fifo.getNumReady(), s1, n1, s2, n2);

        g.setColour (juce::Colour (0xFF00D1FF).withAlpha (0.55f));

        auto drawSlice = [&](int offset, int count)
        {
            for (int i = 0; i < count; ++i)
            {
                const float px = cx + buf[offset + i].l * hx;
                const float py = cy - buf[offset + i].r * hy;
        ...
        // Hairline border
        g.setColour (juce::Colour (0xFF00D1FF).withAlpha (0.18f));
        g.drawRoundedRectangle (b.reduced (0.5f), 4.0f, 0.75f);
    }

private:
    OraclePadAudioProcessor& processor;
};
...
public:
    ...
    LissajousOscilloscope lissajousScope;
