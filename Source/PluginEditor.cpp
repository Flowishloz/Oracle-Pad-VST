      ...
      radarComponent    (p.apvts, p.currentOutputLevel)
{
    ...
    // ── LissajousOscilloscope ─────────────────────────────────────────────────
    addAndMakeVisible (lissajousScope);
...
void OraclePadAudioProcessorEditor::resized()
{
    ...
    // ── Lissajous scope: fills the gap between OSC1 right edge and Radar left ─
    // Computed dynamically so it tracks any future OSC1 / Radar repositioning.
    {
        const int scopeX = kOsc1X + kOsc1W + 4;
        const int scopeY = kOsc1Y + 4;
        const int scopeW = kRadarX - (kOsc1X + kOsc1W) - 8;
        const int scopeH = kOsc2Y  - kOsc1Y - 8;
        lissajousScope.setBounds (scopeX, scopeY, scopeW, scopeH);
    }
