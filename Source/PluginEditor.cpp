      ...
      radarComponent    (p.apvts, p.currentOutputLevel)
{
    ...
    // ── OSC 1 knobs — 5 wired: vol/shape/mix/cut/dcmt ────────────────────────
    setupKnob (osc1VolKnob,   "Osc 1 volume");
    setupKnob (osc1ShapeKnob, "SHAPE: waveform morph — sine→tri→saw→square");
    setupKnob (osc1MixKnob,   "MIX: per-voice micro-detune intensity");
    setupKnob (osc1CutKnob,   "CUT: 2-pole resonant LP filter cutoff");
    setupKnob (osc1DcmtKnob,  "DCMT: 90s bitcrush+decimation dry/wet blend");
    setupLabel (osc1VolLbl,   "VOL");
    setupLabel (osc1ShapeLbl, "SHPE");
    setupLabel (osc1MixLbl,   "MIX");
    setupLabel (osc1CutLbl,   "CUT");
    setupLabel (osc1DcmtLbl,  "DCMT");

    aOsc1Vol   = std::make_unique<SliderAttachment> (p.apvts, "osc1_vol",   osc1VolKnob);
    aOsc1Shape = std::make_unique<SliderAttachment> (p.apvts, "osc1_shape", osc1ShapeKnob);
    ...
    aOsc1Dcmt  = std::make_unique<SliderAttachment> (p.apvts, "osc1_dcmt",  osc1DcmtKnob);
...
OraclePadAudioProcessorEditor::~OraclePadAudioProcessorEditor()
{
    ...
    for (auto* s : { &osc1VolKnob,  &osc1ShapeKnob, &osc1MixKnob,
                     &osc1CutKnob,  &osc1DcmtKnob,
...
void OraclePadAudioProcessorEditor::resized()
{
    ...
    // Bottom row (2 knobs): SPREAD / CUT — centered under top 3 at columns 0.5,1.5
    {
        ...
        topKnob (osc1ShapeKnob, osc1ShapeLbl, 1);
        topKnob (osc1MixKnob,   osc1MixLbl,   2);

        botKnob (osc1CutKnob,  osc1CutLbl,  0);
        botKnob (osc1DcmtKnob, osc1DcmtLbl, 1);
    }
...
{
    static const char* names[] = { "Rainforest", "Temple", "Valley", "City", "Silo" };  // matches AtmosphereCloud::kPresets[0..4]
