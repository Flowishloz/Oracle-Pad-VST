...
void OraclePadAudioProcessor::prepareToPlay (double sampleRate, int samplesPerBlock)
{
    ...
    const auto hpfCoeffs = juce::IIRCoefficients::makeHighPass (sampleRate, 35.0);
    for (auto& f : masterHPF_) { f.reset(); f.setCoefficients (hpfCoeffs); }
}
                                              ...
                                              juce::MidiBuffer& midiMessages)
{
    ...
    // ── Push OSC 1 stereo output to Lissajous FIFO (pre-effects) ───────────
    {
        int s1, n1, s2, n2;
        scopeFifo.prepareToWrite (numSamples, s1, n1, s2, n2);
        const float* chL = oscBuf_.getReadPointer (0);
        const float* chR = oscBuf_.getReadPointer (1);
        for (int i = 0; i < n1; ++i) scopeBuffer[s1 + i] = { chL[i],      chR[i]      };
        for (int i = 0; i < n2; ++i) scopeBuffer[s2 + i] = { chL[n1 + i], chR[n1 + i] };
    ...
    // ── ADSR Gating: gate granular into Main Pad Path ──────────────────────
    for (int s = 0; s < numSamples; ++s)
    {
    ...
    // ── Master Bus: 35 Hz HPF → -6 dB headroom trim → tanh soft-clip ──────
    for (int ch = 0; ch < buffer.getNumChannels() && ch < 2; ++ch)
    {
        float* data = buffer.getWritePointer (ch);
        masterHPF_[ch].processSamples (data, numSamples);
        for (int s = 0; s < numSamples; ++s)
        {
            data[s] *= 0.5f;
