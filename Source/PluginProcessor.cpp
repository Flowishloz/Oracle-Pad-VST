...
juce::AudioProcessorValueTreeState::ParameterLayout OraclePadAudioProcessor::createParameterLayout()
{
    ...
    params.push_back (std::make_unique<juce::AudioParameterFloat> ("osc1_shape", "Shape",       0.0f,  1.0f,  0.5f));
    ...
    params.push_back (std::make_unique<juce::AudioParameterFloat> ("osc1_dcmt",  "DCMT",        0.0f,  1.0f,  0.0f));
                                              ...
                                              juce::MidiBuffer& midiMessages)
{
    ...
    const float shape   = apvts.getRawParameterValue ("osc1_shape") ->load();
    const float mix     = apvts.getRawParameterValue ("osc1_mix")   ->load();
    const float cutHz   = apvts.getRawParameterValue ("osc1_cut")   ->load();
    const float dcmt    = apvts.getRawParameterValue ("osc1_dcmt")  ->load();
    ...
    for (int s = 0; s < numSamples; ++s)
    {
        ...
        for (auto& v : voices)
        {
            auto [vl, vr] = v.renderStereo (shape, mix, sr_f);
            l += vl;  r += vr;
        }
    ...
    // ── CUT filter: 2-pole resonant SVF, Q rises as cutHz falls ────────────
    // normCut=1 at 20 kHz (Q=0.707 Butterworth), normCut=0 at 20 Hz (Q=4.0).
    {
        const float Q    = ResonantLP2Pole::dynamicQ (cutHz);
        float* dataL     = oscBuf_.getWritePointer (0);
        float* dataR     = oscBuf_.getWritePointer (1);
        for (int s = 0; s < numSamples; ++s)
        {
            dataL[s] = cutFilter_[0].tickLP (dataL[s], cutHz, Q, sr_f);
            dataR[s] = cutFilter_[1].tickLP (dataR[s], cutHz, Q, sr_f);
        }
    }

    // ── DCMT: 90s hardware layer — 12-bit crush + 8× ZOH decimation ────────
    // Parallel path: gritty signal blended against the pristine CUT output.
    if (dcmt > 0.001f)
    {
        float* dataL = oscBuf_.getWritePointer (0);
        float* dataR = oscBuf_.getWritePointer (1);
        for (int s = 0; s < numSamples; ++s)
        {
            if (dcmtCounter_ <= 0)
            {
                // 12-bit reduction: quantise to 4096 levels, no dither
                dcmtHeld_[0] = std::round (dataL[s] * 2047.0f) / 2047.0f;
                dcmtHeld_[1] = std::round (dataR[s] * 2047.0f) / 2047.0f;
                dcmtCounter_ = 8;  // ZOH holds each new sample for 8 output frames
            }
            --dcmtCounter_;
            dataL[s] = dataL[s] * (1.0f - dcmt) + dcmtHeld_[0] * dcmt;
            dataR[s] = dataR[s] * (1.0f - dcmt) + dcmtHeld_[1] * dcmt;
        }
    }
